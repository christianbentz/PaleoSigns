...
selected.sequences <- ("XXXXXYYY_X_XXXX_XVXXY_°°°_°°°°°°°_XXXX_>>>XXX_⊞_XXXXX",
                       )
estimations.df.illustration <- estimations.df.selected[estimations.df.selected$sequence == "XXXXXYYY_X_XXXX_XVXXY_°°°_°°°°°°°_XXXX_>>>XXX_⊞_XXXXX" |
estimations.df.selected$sequence ==                                           , ]
```

# Illustration Plots
```{r, fig.width = 5, fig.height = 5, warning = FALSE}
huni.hrate.chars.plot <- ggplot(estimations.df.illustration, aes(x = huni.chars, y =  hrate.chars, color = subcorpus)) + 
