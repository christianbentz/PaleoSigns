...
for (i in 1:nrow(sign.data)){
  ...
  # this is here done by first splitting them on ' _ ' to get strings of characters
  # the strings are then split into individual UTF-8 characters (white spaces included)
  strings <- unlist(strsplit(coding.sample, " _ ")) 
  chars <- unlist(strsplit(strings, ""))
  strings.random <- unlist(strsplit(coding.sample.random, " _ ")) 
  chars.random <- unlist(strsplit(strings.random, ""))
...
Give head of estimation data frame (selected columns) for visual checking of the output data structure. Note that "coding_clean" gives the sequences with underscores.
```{r}
...
```{r}
write.csv(sign.data.est, "~/Github/UpperPalCombinatorics/results/features/signBase_features.csv", row.names = F)
```
